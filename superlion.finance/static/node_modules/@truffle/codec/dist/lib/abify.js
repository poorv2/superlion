...
var debug = debug_1.default("codec:abify");
...
function abifyType(dataType, userDefinedTypes) {
    switch (dataType.typeClass) {
        ...
        case "struct":
            {
                var fullType = Format.Types.fullType(dataType, userDefinedTypes);

                if (!fullType) {
                    var typeToDisplay = Format.Types.typeString(dataType);
                    throw new Common.UnknownUserDefinedTypeError(dataType.id, typeToDisplay);
                }

                var memberTypes = fullType.memberTypes.map(function(_ref) {
                    var name = _ref.name,
        ...
        case "enum":
            {
                var _fullType = Format.Types.fullType(dataType, userDefinedTypes);

                if (!_fullType) {
                    var _typeToDisplay = Format.Types.typeString(dataType);

                    throw new Common.UnknownUserDefinedTypeError(dataType.id, _typeToDisplay);
                }

                var numOptions = _fullType.options.length;
                var bits = 8 * Math.ceil(Math.log2(numOptions) / 8);
                return {
                    typeClass: "uint",
                    bits: bits,
                    typeHint: Format.Types.typeString(_fullType)
                };
            }
            //finally: arrays
    ...
    switch (result.type.typeClass) {
        ...
        case "function":
            switch (result.type.visibility) {
                case "external":
                    {
                        var _coercedResult = result;
                        return Object.assign(Object.assign({}, _coercedResult), {
                            type: abifyType(result.type, userDefinedTypes)
                        });
                    }
        ...
        case "struct":
            {
                var _coercedResult2 = result;

                switch (_coercedResult2.kind) {
                    case "value":
                        if (_coercedResult2.reference !== undefined) {
                            return undefined; //no circular values in the ABI!
                        }

                        var abifiedMembers = _coercedResult2.value.map(function(_ref2) {
                            var name = _ref2.name,
                                member = _ref2.value;
                            return {
                                name: name,
                    ...
                    case "error":
                        return Object.assign(Object.assign({}, _coercedResult2), {
        ...
        case "enum":
            {
                ...
                //be aware!
                var _coercedResult3 = result;
                var uintType = abifyType(result.type, userDefinedTypes); //may throw exception

                switch (_coercedResult3.kind) {
                    case "value":
                        return {
                            type: uintType,
                            kind: "value",
                            value: {
                                asBN: _coercedResult3.value.numericAsBN.clone()
                            }
                        };

                    case "error":
                        switch (_coercedResult3.error.kind) {
                            case "EnumOutOfRangeError":
                                return {
                                    type: uintType,
                                    kind: "value",
                                    value: {
                                        asBN: _coercedResult3.error.rawAsBN.clone()
                                    }
                                };
                            ...
                            case "EnumPaddingError":
                                return {
                                    ...
                                    error: {
                                        ...
                                        paddingType: _coercedResult3.error.paddingType,
                                        raw: _coercedResult3.error.raw
                                    }
                                };

                            case "EnumNotFoundDecodingError":
                                var numericValue = _coercedResult3.error.rawAsBN.clone();
        ...
        case "array":
            {
                var _coercedResult4 = result;

                switch (_coercedResult4.kind) {
                    case "value":
                        if (_coercedResult4.reference !== undefined) {
                            return undefined; //no circular values in the ABI!
                        }

                        var _abifiedMembers = _coercedResult4.value.map(function(member) {
                            return abifyResult(member, userDefinedTypes);
                        });

                        return {
                            kind: "value",
                            type: abifyType(result.type, userDefinedTypes),
                            value: _abifiedMembers
                        };

                    case "error":
                        return Object.assign(Object.assign({}, _coercedResult4), {
                            type: abifyType(result.type, userDefinedTypes)
                        });
                }
            }
    ...
    switch (decoding.kind) {
        ...
        case "constructor":
            return Object.assign(Object.assign({}, decoding), {
                decodingMode: "abi",
                arguments: decoding.arguments.map(function(argument) {
                    return Object.assign(Object.assign({}, argument), {
