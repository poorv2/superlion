...
function mathematica(hljs) {
    ...
    var BASE_NUMBER_RE = either(concat(BASE_RE, BASE_DIGITS_RE), NUMBER_RE);
    var ACCURACY_RE = /``[+-]?(\d*\.\d+|\d+\.\d*|\d+)/;
    var PRECISION_RE = /`([+-]?(\d*\.\d+|\d+\.\d*|\d+))?/;
    var APPROXIMATE_NUMBER_RE = either(ACCURACY_RE, PRECISION_RE);
    var SCIENTIFIC_NOTATION_RE = /\*\^[+-]?\d+/;
    var MATHEMATICA_NUMBER_RE = concat(BASE_NUMBER_RE, optional(APPROXIMATE_NUMBER_RE), optional(SCIENTIFIC_NOTATION_RE));
    var NUMBERS = {
        className: 'number',
        relevance: 0,
